...
private:
    ...
    //Shoot
    // TODO ENFRIAMIENTO BALAS CON RELOJ
    static void
    space_pressed(std::unique_ptr<EntityPlayer> &player) { if (CooldownSystem::shootReady()) player->shooting = true; }
    ...
    static void m_pressed(std::unique_ptr<EntityPlayer> &player) {
        ...
        player->mode ? player->renderable.texture = "./img/textures/testing/testing_angel.jpg"
                     : player->renderable.texture = "./img/textures/testing/testing_demon.jpg";
        player->node.setTexture(player->renderable.texture);
    }
