...
vec3 InputSystem::cursorCoordToWorldCoord(float x, float y, float far) const {
    ...
    const auto mview = device.getInnerDevice()->getSceneManager()->getActiveCamera()->getViewMatrix();
    ...
    const auto viewport = device.getInnerDevice()->getVideoDriver()->getViewPort();
    ...
    const glm::vec4 viewportPos (x / viewport.getWidth() * 2.0 - 1, y / viewport.getHeight() * 2.0 - 1.0, far * 2.0 - 1.0, 1.0);
