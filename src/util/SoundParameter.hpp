...
constexpr sound_parameter_tuple getSoundTuple(const SoundParameter p){
	switch(p){
		...
		//PLAYER
		case ATTACK_PLAYER_DEMON :      return { PLAYER_SHOOT_EVENT, "mode", DEMON };
		case ATTACK_PLAYER_ANGEL :      return { PLAYER_SHOOT_EVENT, "mode", ANGEL };

		case SWITCH_MODE_DEMON :        return { CHANGE_MODE_EVENT,  "mode", DEMON };
		case SWITCH_MODE_ANGEL :        return { CHANGE_MODE_EVENT,  "mode", ANGEL };
