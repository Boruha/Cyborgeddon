#pragma once

#include <string_view>

/* DEX */
constexpr std::string_view MESH_DEX_STATIC              = "resources/models/Dex/Static/0.obj";
constexpr std::string_view ANIMATED_DEX_WALKING         = "resources/models/Dex/Walking";
constexpr std::string_view ANIMATED_DEX_STATIC_ATTACK   = "resources/models/Dex/StaticAttack";
constexpr std::string_view ANIMATED_DEX_WALKING_ATTACK  = "resources/models/Dex/WalkingAttack";
constexpr std::string_view MESH_DEX_DASHING             = "resources/models/Dex/Dash/14.obj";
constexpr std::string_view ANIMATED_DEX_DEATH           = "resources/models/Dex/Death";

/* BOT */
constexpr std::string_view MESH_BOT_STATIC        = "resources/models/Bot/Static/0.obj";
constexpr std::string_view ANIMATED_BOT_WALKING   = "resources/models/Bot/Walking";
constexpr std::string_view ANIMATED_BOT_DEATH     = "resources/models/Bot/Death";
constexpr std::string_view ANIMATED_BOT_ATTACK    = "resources/models/Bot/Attack";

/* ANGEL */
constexpr std::string_view MESH_ANGEL_STATIC      = "resources/models/Angel/Static/0.obj";
constexpr std::string_view ANIMATED_ANGEL_WALKING = "resources/models/Angel/Walking";
constexpr std::string_view ANIMATED_ANGEL_DEATH   = "resources/models/Angel/Death";
constexpr std::string_view ANIMATED_ANGEL_ATTACK  = "resources/models/Angel/Attack";

/* DEMON */
constexpr std::string_view MESH_DEMON_STATIC      = "resources/models/Demon/Static/0.obj";
constexpr std::string_view ANIMATED_DEMON_WALKING = "resources/models/Demon/Walking";
constexpr std::string_view ANIMATED_DEMON_DEATH   = "resources/models/Demon/Death";
constexpr std::string_view ANIMATED_DEMON_ATTACK  = "resources/models/Demon/Attack";
constexpr std::string_view ANIMATED_DEMON_JUMP    = "resources/models/Demon/Jump";

/* CITY */
constexpr std::string_view MESH_CITY      = "resources/models/Ciudad/ciudad.obj";
constexpr std::string_view COLISIONS_CITY = "resources/models/Ciudad/colisiones.obj";

/* BULLET */
constexpr std::string_view ANIMATED_BALA   = "resources/models/Objetos/Bala/BalaCentro";

/* DOOR */
constexpr std::string_view MESH_DOOR_OPEN   = "resources/models/Objetos/Puerta/0.obj";
constexpr std::string_view ANIMATED_DOOR    = "resources/models/Objetos/Puerta";

/* KEYS */
constexpr std::string_view MESH_KEY_DEMON      = "resources/models/Objetos/Llave_demoniaca/0.obj";
constexpr std::string_view MESH_KEY_ANGEL      = "resources/models/Objetos/Llave_angelical/0.obj";
